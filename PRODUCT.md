...
## Data integrity
Every ocean value is deterministic synthetic demonstration data. Temperature, salinity, sea level, wind, currents, reference curves, and sample stations are illustrative; they are not satellite observations, ARGO measurements, or trained model predictions. The interface omits demo badges and dataset warnings at the user’s request; technical documentation and exported CSV retain data provenance. The inspector's profile and history remain temperature charts; regional trends follow the selected variable. Early January history contains only the available dates.
...
## Readability and globe interaction
Supporting typography was increased across all three routes. Landing and architecture prose use approximately 16px, with 12–14px metadata; dashboard labels use 13–14px and compact chart labels 12px. The Earth uses the original natural-color texture and supports drag, arrow keys, rotate, and reset controls. Map wheel and trackpad zoom preserve the geographic point under the cursor, normalize wheel units, and support pinch gestures.
